...
=======
```{r, include = FALSE}
>>>>>>> e1f15924acf0ab85c078b89ec557a53835d37d16
library(fixest)
library(ggplot2)
library(tidyverse)
library(vtable)
...
time_full <- clean_df %>% group_by(retail_or_not, monthyear) %>% summarize(n1 = sum(employed_full)/tot)  # divide by in order to get proportions
...
From the Marginal Effect we see that the impact is fairly low, and has little to no significance.



Final Model Logit with Interaction Term:
For the final Model I added in the extra set of controls (Age, Sex, Race), which we will verify are still useful with a WALD test.
Additionally, I create another dummy variable for April, to see what the interaction is between being before and after April is on Retail employment compared to other industries.
```{r}
...
summary(predictions(ques2logit2, newdata = datagrid(retail_or_not= 0, covid_april = 0)))
summary(predictions(ques2logit2, newdata = datagrid(retail_or_not= 1, covid_april = 0)))
summary(predictions(ques2logit2, newdata = datagrid(retail_or_not= 0, covid_april = 1)))
...
avg_unemper
```
We see that retail_or_not increases probability of being unemployed by 1.38%, which is ontop of the average unemployment rate of 3.23% for the sample. 

In our interaction term we see that if the time is in April 2020 or later that there is an increased....


```{r}
#if the above model isn't feglm, then this doesn't work right
...
```
Interpreting this model based on the interaction term we see that in the Marginal Effect there is a 1.9% increase probability in being unemployed  as a retail worker. 
...
-Assumptions about employment types?
-Assumptions about other industries?
...
<<<<<<< HEAD
-It would be useful to have data on Sales Revenue from retail stores, and what the customer demographic was

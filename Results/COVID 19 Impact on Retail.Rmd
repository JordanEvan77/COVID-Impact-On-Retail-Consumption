...
ques2logit2 <- feglm(unemper ~ retail_or_not + retail_or_not*covid_april + YEAR + MONTH | REGION + covid1_dummy + 
...
```{r}
pred <- predictions(ques2logit2, variables = list(retail_or_not = c(0, 1), april_covid= c(0,1)))
head(pred)

```

Interpretted Interaction Term:
```{r}
int_eff <- intEff(ques2logit2, c("retail_or_not", "covid_april"), ques2)
summary(int_eff)

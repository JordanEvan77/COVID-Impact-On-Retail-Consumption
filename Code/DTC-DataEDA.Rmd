...
sean_data <- first_data %>% 
  dplyr::select(REGION, WTFINL, NCHILD, CLASSWKR, COVIDUNAW, FAMINC, IncNumber, DIFFANY, YEAR, SERIAL, MONTH, HWTFINL, CPSID, YEAR, SERIAL, MONTH, HWTFINL)
...
sean_data %>% count(REGION)
sean_data %>% count(CLASSWKR)
sean_data %>% count(NCHILD)
sean_data %>% count(IncNumber)
sean_data %>% count(DIFFANY)
sean_data %>% count(YEAR)
sean_data %>% count(COVIDUNAW)

```
```{r}

region_class <- sean_data %>% count(REGION,CLASSWKR)
region_class

ggplot(region_class, aes(x = REGION, y = n, fill = CLASSWKR)) +
  geom_col() +
  coord_flip()
...
Inc_Chld <- sean_data %>% count(IncNumber, NCHILD)
Inc_Chld
Inc_Chld$NCHILD <- as.factor(Inc_Chld$NCHILD)

ggplot(Inc_Chld, aes(x = IncNumber, y = n, fill = NCHILD)) +
  geom_col()
...
reg_Chld <- sean_data %>% count(REGION, NCHILD)
reg_Chld

sort(sean_data$REGION) %>% unique()

reg_Chld$NCHILD <- as.factor(reg_Chld$NCHILD)

ggplot(reg_Chld, aes(x = REGION, y = n, fill = NCHILD)) +
  geom_col() +
  coord_flip()
...
diff_inc <- sean_data %>% count(DIFFANY, IncNumber)
diff_inc$IncNumber <- as.factor(diff_inc$IncNumber)
diff_inc

ggplot(diff_inc %>% filter(DIFFANY == 'Has difficulty'), aes(x = IncNumber, y = n)) +
  geom_col()

ggplot(diff_inc %>% filter(DIFFANY == 'No difficulty'), aes(x = IncNumber, y = n)) +
  geom_col()
...
diff_class <- sean_data %>% count(DIFFANY, CLASSWKR)
diff_class

ggplot(diff_class %>% filter(DIFFANY == 'Has difficulty'), aes(x = CLASSWKR, y = n)) +
  geom_col() +
  coord_flip()

ggplot(diff_class %>% filter(DIFFANY == 'No difficulty'), aes(x = CLASSWKR, y = n)) +
  geom_col() +
  coord_flip()
...
covun_class <- sean_data %>% count(COVIDUNAW, CLASSWKR)
covun_class

covun_class <- covun_class %>% 
  mutate(total = case_when(COVIDUNAW == 'No' ~ 1222494,
                           COVIDUNAW == 'Yes' ~ 75946,
                           TRUE ~ 1624904)) %>% 
...
ggplot(covun_class %>% filter(COVIDUNAW == 'No'), aes(x = CLASSWKR, y = proportion)) +
  geom_col() +
  coord_flip()

ggplot(covun_class %>% filter(COVIDUNAW == 'Yes'), aes(x = CLASSWKR, y = proportion)) +
  geom_col() +
  coord_flip()
...
covun_inc <- sean_data %>% count(COVIDUNAW, IncNumber)
covun_inc$IncNumber <- as.factor(covun_inc$IncNumber)
...
ggplot(covun_inc %>% filter(COVIDUNAW == 'No'), aes(x = IncNumber, y = proportion)) +
  geom_col()

ggplot(covun_inc %>% filter(COVIDUNAW == 'Yes'), aes(x = IncNumber, y = proportion)) +
  geom_col()
...
andy_data <- first_data %>% 
  dplyr:: select(STATEFIP, CPSIDP, CITIZEN, WHYUNEMP, COVIDLOOK, RACE, IND, INDNAME, PERNUM, YEAR, MONTH, monthyear)
...
#------------------
# Histogram for IND
RetailTradeHist <- andy_data %>%
  mutate(RetailOrNot = ifelse(INDNAME %in% c('Retail Trade'), "Retail Trade","Other Industry")) %>%
  ggplot(mapping = aes(RetailOrNot)) + 
  geom_histogram(stat = 'count',  fill="cadetblue") +
  labs(x = 'Industry Name',
       y = 'Count of Individuals',
       title = 'Respondent Count by Industry Name') +
  theme(axis.text.x = element_text(angle = -45),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_blank(),
...
ggsave(filename = "../Plots/Whyunemp.png", plot = whyunemp_chart)
```


Changes over time:
```{r}
whyunemp_retail <- andy_data %>%
  mutate(RetailOrNot = ifelse(INDNAME %in% c('Retail Trade'), 
                              "Retail Trade","Other Industry")) %>%
  group_by(WHYUNEMP, monthyear, RetailOrNot) %>%
  summarize(Count = sum(Count)) %>%
  filter(WHYUNEMP != 'NIU')
  
whyunemp_retail_chart <- whyunemp_retail%>%
  ggplot(mapping = aes(x = monthyear, y = Count, color = as.factor(RetailOrNot))) +
  geom_line() +
  labs(x = 'Reason Why Unemployed',
       y = 'Aggregated Count',
       title = 'Unemployment Reason Changes for Retail and Non-Retail Over Time',
       subtitle = 'Excludes NIU (no response)') +
  theme(legend.position = 'bottom',
        axis.text.x = element_text(angle = -45),
        panel.grid.major = element_blank(),
...
whyunemp_retail_chart # view plot
```
...
brooke_data <- first_data %>% 
  dplyr::select(METAREA, AGE, NATIVITY, WKSTAT, INDNAME, EMPSTAT, MARST, monthyear)

```



#Super Categorical LFG!!
```{r}


brooke_data %>% ggplot(mapping = aes(METAREA)) + 
  geom_histogram(stat = 'count', fill="blue") +
  coord_flip()

brooke_data %>% ggplot(mapping = aes(AGE)) + 
  geom_histogram(stat = 'count', fill="blue") 

brooke_data %>% ggplot(mapping = aes(NATIVITY)) + 
  geom_histogram(stat = 'count', fill="blue")+
  coord_flip()

brooke_data %>% ggplot(mapping = aes(WKSTAT)) + 
  geom_histogram(stat = 'count', fill="blue") +
  coord_flip()

brooke_data %>% ggplot(mapping = aes(INDNAME)) + 
  geom_histogram(stat = 'count', fill="blue") +
  coord_flip()

brooke_data %>% ggplot(mapping = aes(EMPSTAT)) + 
  geom_histogram(stat = 'count', fill="blue") +
  coord_flip()

brooke_data %>% ggplot(mapping = aes(MARST)) + 
  geom_histogram(stat = 'count', fill="blue") +
  coord_flip()
...
```{r}
hisp_time <- jordan_data %>% group_by(HISPAN, monthyear) %>% summarise(n1= n())
...
educ_time <- jordan_data %>% group_by(EDUC, monthyear) %>% summarise(n2= n())
...
sch_time <- jordan_data %>% group_by(SCHLCOLL, monthyear) %>% summarise(n3= n())
...
```{r}
#Now looking at how Full employment has faired in and out of retail.

clean_df$employed_full <- ifelse(clean_df$WKSTAT %in% c("Full-time hours (35+), usually full-time", "Full-time hours (35+), usually part-time for economic reasons", "Full-time hours (35+), usually part-time for non-economic reasons"), 1, 0)

clean_df$retail_or_not <- ifelse(clean_df$INDNAME %in% c("Retail Trade"), 1, 0)

tot = clean_df %>%  summarize(n = sum(employed_full)) %>% pull(n)




time_full <- clean_df %>% group_by(retail_or_not, monthyear) %>% summarize(n1 = sum(employed_full)/tot)

timer_full <- time_full %>% 
  ggplot(mapping = aes(x = monthyear, y = n1, color = as.factor(retail_or_not))) + 
  geom_line() +
  labs(x = 'Year and Month',
       y = 'Proportion of Full Time Employment',
       title = 'Retail suffered less, but also started with less full time employment',
       subtitle = '1 (blue) is retail')
...
```{r}
ggsave(filename = "../Plots/fulltime_retail.png", plot = timer_full)
```
